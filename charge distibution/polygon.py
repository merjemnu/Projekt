...
def calculate_potential(x_grid, y_grid):
    ...
    # Sum the contributions from each point in the charge distribution (uniform)
    for i in range(x_grid.shape[0]):
        for j in range(x_grid.shape[1]):
            if geometry(x_grid[i, j], y_grid[i, j]):  # Check if inside the object
                for x_c in range(x_grid.shape[0]):
                    for y_c in range(x_grid.shape[1]):
                        if geometry(x_grid[x_c, y_c], y_grid[x_c, y_c]):
                            potential[i, j] += greens_function(x_grid[i, j], y_grid[i, j], x_grid[x_c, y_c], y_grid[x_c, y_c])
